        ...
        if (isset($_SESSION['dbid'])) {
            ...
            if ($result->num_rows > 0) {
                ...
                if ($lic[$num] == '1') {
                    $lic[$num] = '0';
                    if ($settings['logging']) {
                        $dao->logAdd($_SESSION['user_name'], $lang['removed'] . ' ' . uIDname($_POST['player'], $db_link) . ' ' . $lang['licenses'] . ' (' . licName($_POST['id'], $license) . ')', 2);
                    }
                } elseif ($lic[$num] == '0') {
                    $lic[$num] = '1';
                    if ($settings['logging']) {
                        $dao->logAdd($_SESSION['user_name'], $lang['added'] . ' ' . uIDname($_POST['player'], $db_link) . ' ' . $lang['licenses'] . ' (' . licName($_POST['id'], $license) . ')', 2);
                    }
                }
