        ...
        if (isset($_SESSION['dbid'])) {
            ...
            if ($result->num_rows > 0) {
                ...
                if ($active == '1') {
                    $active = '0';
                    if ($settings['logging']) {
                        $dao->logAdd($_SESSION['user_name'], $lang['edited'] . ' ' . uIDname($_POST['player'], $db_link) . ' ' . $lang['gang'] . ' ' . $lang['deactive'], 1);
                    }
                } elseif ($active == '0') {
                    $active = '1';
                    if ($settings['logging']) {
                        $dao->logAdd($_SESSION['user_name'], $lang['edited'] . ' ' . uIDname($_POST['player'], $db_link) . ' ' . $lang['gang'] . ' ' . $lang['active'], 1);
                    }
                } else {
                    $active = '0';
                }
